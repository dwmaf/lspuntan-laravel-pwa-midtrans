<div x-data="{ open: false }" >
    <aside class="h-full  bg-white dark:bg-gray-800 p-3 transform transition-all duration-300 "
        ...
        <!-- Sidebar Content -->
        <h6 class="text-gray-700 mt-10 dark:text-gray-200" :class="!open ? 'hidden' : ''">Selamat datang, {{ auth()->user()->name }}</h6>
        ...
        @if (auth()->user()->role == 'asesor' || auth()->user()->role == 'admin')
            <a href="{{ route('dashboardadmin') }}"
                ...
                :class="[(open && location.pathname === '/dashboardadmin') ? 'bg-gray-800' : '',!open? 'mt-10':'']">
                <x-bxs-grid-alt class="w-5 text-gray-700 dark:text-gray-200"/>
                
                <span class="text-gray-700 dark:text-gray-200 rounded-sm"
                    :class="!open ? 'hidden' : ''">
                    Dashboard
                </span>
            </a>
        ...
        @endif
        <form method="POST" action="{{ route('logout') }}" class="gap-2 flex items-center w-full px-3 py-2 text-gray-700 dark:text-gray-200 hover:bg-gray-200 dark:hover:bg-gray-700 rounded-sm cursor-pointer">
            @csrf
