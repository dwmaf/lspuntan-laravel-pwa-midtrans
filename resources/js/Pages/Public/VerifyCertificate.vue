...
<template>
    ...
    <div class="font-sans text-gray-900 dark:text-gray-100 antialiased min-h-screen bg-gray-100 dark:bg-gray-900 flex flex-col items-center pt-6 sm:pt-10">
        <div>
            ...
            <div v-if="certificate" class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700">
                <div class="flex flex-col items-center text-center">
                    <div class="p-3 bg-green-100 dark:bg-green-900 rounded-full">
                        <svg class="w-12 h-12 text-green-600 dark:text-green-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                    </div>
                ...
                <div class="mt-6 text-sm">
                    <dl class="space-y-3">
                        <div class="flex justify-between">
                            ...
                            <dd class="font-semibold text-gray-900 dark:text-white text-right">{{ certificate.asesi.student.user.name }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="font-medium text-gray-500 dark:text-gray-400">Skema Sertifikasi</dt>
                            <dd class="text-gray-700 dark:text-gray-300 text-right">{{ certificate.asesi.sertification.skema.nama_skema }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="font-medium text-gray-500 dark:text-gray-400">Nomor Sertifikat</dt>
                            <dd class="font-mono text-gray-700 dark:text-gray-300 text-right">{{ certificate.nomor_sertifikat }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="font-medium text-gray-500 dark:text-gray-400">Nomor Registrasi</dt>
                            <dd class="font-mono text-gray-700 dark:text-gray-300 text-right">{{ certificate.nomor_registrasi }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="font-medium text-gray-500 dark:text-gray-400">Tanggal Terbit</dt>
                            <dd class="text-gray-700 dark:text-gray-300 text-right">{{ formatDate(certificate.tanggal_terbit) }}</dd>
                        </div>
                        <div class="flex justify-between">
                            <dt class="font-medium text-gray-500 dark:text-gray-400">Berlaku Hingga</dt>
                            <dd class="text-gray-700 dark:text-gray-300 text-right">{{ formatDate(certificate.berlaku_hingga) }}</dd>
                        </div>
                    </dl>
                </div>
            </div>
